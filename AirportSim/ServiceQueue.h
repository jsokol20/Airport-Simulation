	...
	void update(int clock)
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {
				...
				the_queue.pop(); //removes the top plane from the service queue

				// FIXME: update the enter_departure_time attribute for the plane
				plane -> enter_departure_time = clock - plane-> start_service_time; //the departure time equals the difference of the clock and the service time
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				int Wait = 0;
				Wait = clock - plane->arrival_time; //Actual time subtracted by arrival time is wait time

				// FIXME: update total_wait and num_served for the landing queue
				landing_queue->num_served++; //keep track of the planes service number (1 each time)
				landing_queue->total_wait += Wait;


				// FIXME: update the start_service_time attribute for the plane
				plane->start_service_time = clock; //Actual time is the same as service time
				...
				*/
				double randDouble = 0;
				while (!(randDouble < min_service_time)) {
					randDouble = (my_random.next_int(max_service_time)); //finding value less than max but greater than min
				};
				plane->service_time = randDouble; //service time is random double value because the plane needs to have a random number to be served
