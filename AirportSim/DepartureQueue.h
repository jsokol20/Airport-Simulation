		...
		if (!the_queue.empty()) {
			...
			if (plane->ready_takeoff_time == -1) { // new plane has arrived at the front
				...
				if (clock - plane->ready_takeoff_time > departure_time) {
					...
					the_queue.pop(); //the plane at the top of the queue gets removed

					// FIXME: calculate the wait time
					int Wait = 0;
					Wait = clock - plane->ready_takeoff_time;

					// FIXME: update total_wait and num_served
					total_wait += Wait; //wait time is added to total time stated in p.393
